...
import { SeatType, StopType, type Route } from '../types';
    ...
    if (editingTrip) {
      reset({
        ...
        stops: (editingTrip.route.stops || []).map(stop => ({
            ...
            durationMinutesFromOrigin: stop.durationMinutesFromOrigin,
            stopType: stop.stopType,
            estimatedArrivalTime: stop.estimatedArrivalTime ? format(new Date(stop.estimatedArrivalTime), 'HH:mm') : '',
    ...
    try {
      ...
      const formattedStops = (data.stops || []).map((stop) => {
        ...
        if (stop.estimatedArrivalTime) {
           ...
           if (stopDate.getTime() < depDate.getTime() && (depDate.getTime() - stopDate.getTime() > 12 * 60 * 60 * 1000)) {
      ...
      const tripPayload = {
        ...
        recurrence: data.tripType === 'RECURRING' ? {
      ...
      <form onSubmit={handleSubmit(onSubmit, (errors) => {
          ...
          const getErrorMessages = (obj: unknown): string[] => {
            ...
            const extract = (item: unknown, depth = 0) => {
                ...
                if (item && 'message' in (item as Record<string, unknown>) && typeof (item as Record<string, unknown>).message === 'string') {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-2">
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><MapPin className="h-5 w-5" /> Thông tin Tuyến & Xe</CardTitle>
                </CardHeader>
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Field>
                      <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Tuyến đường</FieldLabel>
                      <Select
                    ...
                    <Field>
                      <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Xe</FieldLabel>
                      <Select
            ...
            <Card>
                 <CardHeader>
                    <CardTitle className="flex items-center gap-2"><Clock className="h-5 w-5" /> Loại Chuyến đi</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="flex flex-col gap-2">
                         <div className="flex items-center space-x-2 border p-3 rounded-md">
                            ...
                            />
                            <Label htmlFor="type-single" className="cursor-pointer flex-1">Chuyến đi đơn</Label>
                        </div>
                        <div className="flex items-center space-x-2 border p-3 rounded-md">
                            <Checkbox 
                                id="type-recurring"
                            ...
                            />
                            <Label htmlFor="type-recurring" className="cursor-pointer flex-1">Lịch lặp lại</Label>
                        </div>
                    </div>
                </CardContent>
            </Card>
        ...
        <Card>
             <CardHeader>
                <CardTitle className="flex items-center gap-2"><Calendar className="h-5 w-5" /> Chi tiết Lịch trình</CardTitle>
            ...
            <CardContent>
                 {isRecurrenceDisabled && (
                    <div className="mb-4 flex items-center gap-2 p-3 bg-red-50 text-red-600 border border-red-200 rounded-md text-sm">
                        ...
                        <span>
                        Có {recurrenceCheck?.futureBookingsCount} đặt vé trong tương lai. Không thể thay đổi cấu hình lặp lại.
                        </span>
                    </div>
                )}
                ...
                {tripType === 'SINGLE' ? (
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Field>
                            <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Ngày khởi hành</FieldLabel>
                             <Popover>
                        ...
                        <Field>
                            <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Giờ khởi hành</FieldLabel>
                            <Input 
                                type="time" 
                                onChange={(e) => {
                    ...
                    <div className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <Field>
                                <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Giờ khởi hành (Hàng ngày)</FieldLabel>
                                <Input 
                                    type="time" 
                                    {...register('departureTime')}
                                    disabled={isRecurrenceDisabled}
                                />
                                {errors.departureTime && <FieldError>{errors.departureTime.message}</FieldError>}
                            </Field>
                             <Field>
                                <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Ngày bắt đầu</FieldLabel>
                                <Popover>
                                    <PopoverTrigger asChild>
                                        <Button
                                            variant={"outline"}
                                            className={cn("w-full pl-3 text-left font-normal", !watch('startDate') && "text-muted-foreground")}
                                            disabled={isRecurrenceDisabled}
                                        >
                                            {watch('startDate') ? format(watch('startDate')!, "PPP") : <span>Chọn ngày</span>}
                                    ...
                                    <PopoverContent className="w-auto p-0" align="start">
                                        <CalendarComponent
                                            ...
                                            disabled={(date) => date < new Date() || (watch('endDate') ? date > watch('endDate')! : false)}
                                            initialFocus
                                        />
                                    </PopoverContent>
                                </Popover>
                             ...
                             <Field>
                                <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Ngày kết thúc</FieldLabel>
                                <Popover>
                                    <PopoverTrigger asChild>
                                        <Button
                                            variant={"outline"}
                                            className={cn("w-full pl-3 text-left font-normal", !watch('endDate') && "text-muted-foreground")}
                                            disabled={isRecurrenceDisabled}
                                        >
                                            {watch('endDate') ? format(watch('endDate')!, "PPP") : <span>Chọn ngày</span>}
                                            <Calendar className="ml-auto h-4 w-4 opacity-50" />
                                        </Button>
                                    </PopoverTrigger>
                                    <PopoverContent className="w-auto p-0" align="start">
                                        <CalendarComponent
                                            ...
                                            disabled={(date) => (watch('startDate') ? date < watch('startDate')! : date < new Date())}
                                            initialFocus
                                        />
                                    </PopoverContent>
                                </Popover>
                        ...
                        <Field>
                            <div className="flex items-center justify-between mb-2">
                                <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500 mb-0">Lặp lại vào các thứ</FieldLabel>
                                <Button
                                    type="button"
                                    variant="ghost"
                                    size="sm"
                                    ...
                                    disabled={isRecurrenceDisabled}
                                >
                                    {(watch('weeklyDays')?.length || 0) === WEEKDAYS.length ? 'Bỏ chọn tất cả' : 'Chọn tất cả (Hằng ngày)'}
                                </Button>
                            </div>
                                    ...
                                    <div key={day.value} className="flex items-center space-x-2">
                                        <Checkbox 
                                            ...
                                            onCheckedChange={(checked) => {
                                                ...
                                                else setValue('weeklyDays', current.filter(d => d !== day.value));
                                            }}
                                            disabled={isRecurrenceDisabled}
                                        />
        ...
        <Card className="border-primary/20 shadow-md">
            ...
            <CardContent className="p-0">
                <div className="p-6 border-b bg-white">
                    <Label className="text-base font-semibold mb-3 block">Giá vé cơ bản (toàn tuyến)</Label>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-6">
                    <div className="flex items-center justify-between mb-4">
                        <Label className="text-base font-semibold">Danh sách Trạm dừng & Giá chi tiết</Label>
                        ...
                        <TableBody>
                          {stopFields.map((field, index) => (
                            <TableRow key={field.id} className="hover:bg-slate-50/50">
                              ...
                              <TableCell>
                                 <div className="font-semibold text-primary">{field.customName || `Trạm ${field.stopOrder}`}</div>
                                 <div className="text-xs text-muted-foreground truncate max-w-[180px]" title={field.customAddress}>{field.customAddress}</div>
                                 <input type="hidden" {...register(`stops.${index}.stationId`)} value={field.stationId || ''} />
                                 <input type="hidden" {...register(`stops.${index}.customName`)} value={field.customName || ''} />
                                 <input type="hidden" {...register(`stops.${index}.customAddress`)} value={field.customAddress || ''} />
                                 <input type="hidden" {...register(`stops.${index}.stopOrder`, { valueAsNumber: true })} value={field.stopOrder} />
                                 <input type="hidden" {...register(`stops.${index}.durationMinutesFromOrigin`, { valueAsNumber: true })} value={field.durationMinutesFromOrigin} />
                              </TableCell>
                              <TableCell>
                                <Select
                                  onValueChange={(value) => setValue(`stops.${index}.stopType`, value as StopType)}
                              ...
                              <TableCell>
                                  <div className="text-sm text-center">{field.durationMinutesFromOrigin} phút</div>
                              </TableCell>
                              <TableCell>
                                  <Input 
                                    type="time" 
                               ...
                               <TableCell>
                                  <div className="relative">
                                      <Input 
                                        type="number" 
                                        className={cn("h-9 pl-7", errors.stops?.[index]?.normalPrice && "border-destructive")}
                              ...
                              <TableCell>
                                  <div className="relative">
                                      <Input 
                                        type="number" 
                                        className={cn("h-9 pl-7", errors.stops?.[index]?.vipPrice && "border-destructive")}
                                        placeholder="Mặc định"
        ...
        <div className="flex justify-end gap-4 sticky bottom-6 bg-background/95 p-4 border rounded-lg shadow-lg backdrop-blur supports-[backdrop-filter]:bg-background/60">
            <Button type="button" variant="outline" size="lg" onClick={() => navigate('/admin/catalog/trips')}>
              Hủy bỏ
            </Button>
