...
export const RouteFormPage = () => {
  ...
  const previewRoute: Route = isEditing && route ? route : {
    id: 'temp',
    name: watchedValues.name,
    originStation: stations?.find(s => s.id === watchedValues.originStationId) || { id: 'mock', name: 'Điểm đi' } as unknown as Station,
    destinationStation: stations?.find(s => s.id === watchedValues.destinationStationId) || { id: 'mock', name: 'Điểm đến' } as unknown as Station,
                ...
                <CardContent>
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <Field data-invalid={!!errors.originStationId}>
                            <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Điểm đi</FieldLabel>
                            <select
                        ...
                        <Field data-invalid={!!errors.destinationStationId}>
                            <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Điểm đến</FieldLabel>
                            <select
                                className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                        ...
                        <Field data-invalid={!!errors.durationMinutes}>
                            <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Thời gian (phút)</FieldLabel>
                            <Input 
                                type="number" 
                        ...
                        <Field data-invalid={!!errors.distanceKm}>
                            <FieldLabel className="after:content-['*'] after:ml-0.5 after:text-red-500">Khoảng cách (km)</FieldLabel>
                            <Input
                                type="number"
                                step="0.1"
