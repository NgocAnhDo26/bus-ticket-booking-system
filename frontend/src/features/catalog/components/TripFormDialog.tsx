...
import { SeatType, StopType, type Route, type Trip } from '../types';
...
export const TripFormDialog = ({ open, onOpenChange, editingTrip, defaultTab = 'basic' }: TripFormDialogProps) => {
    ...
    if (open) {
      if (editingTrip) {
        reset({
          ...
          stops: (editingTrip.route.stops || []).map(stop => { // Use route stops or trip stops?
  ...
  const isRecurrenceDisabled = Boolean(editingTrip && recurrenceCheck && !recurrenceCheck.canUpdate);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Điền thông tin chi tiết cho chuyến đi.
          </DialogDescription>
        </DialogHeader>
            ...
            <TabsContent value="schedule" className="space-y-6">
                ...
                {isRecurrenceDisabled && (
                    <div className="flex items-center gap-2 p-3 bg-destructive/10 text-destructive rounded-md text-sm">
                        ...
                        <span>
                        Có {recurrenceCheck?.futureBookingsCount} đặt vé trong tương lai. 
                        Không thể thay đổi cấu hình lặp lại.
                        </span>
                    </div>
                )}
                         ...
                         <Popover>
                            ...
                            <div className="p-3 border-t">
                                ...
                                <Input 
                                    ...
                                    value={watch('departureDate') ? format(watch('departureDate')!, 'HH:mm') : ''}
                                />
                            </div>
                            </PopoverContent>
                          </Popover>
                    ...
                    <div className="space-y-4">
                        ...
                        <Field>
                            <div className="flex items-center justify-between mb-2">
                                <FieldLabel required className="mb-0">Lặp lại hàng tuần</FieldLabel>
                                <Button
                                    type="button"
                                    variant="ghost"
                                    size="sm"
                ...
                <TableBody>
                  {stopFields.map((field, index) => (
                    <TableRow key={field.id}>
                      <TableCell>
                         <div className="font-medium">{field.customName || `Trạm ${field.stopOrder}`}</div>
                  ...
                  {stopFields.length === 0 && (
                      <TableRow>
                          <TableCell colSpan={6} className="text-center text-muted-foreground">
                              Chưa có trạm dừng nào. Vui lòng thêm trạm vào tuyến đường hoặc chọn tuyến đường khác.
                          </TableCell>
                      </TableRow>
                  )}
                </TableBody>
