...
export const BookingPage = () => {
  ...
  const { initialize, selectedSeats, pickupStationId, dropoffStationId, setPickupStationId, setDropoffStationId } = useBookingStore(
    useShallow((state) => ({
      ...
      if (tripId) {
        ...
        if (pendingBookingId) {
           try {
              ...
              const booking = await bookingApi.getBookingById(pendingBookingId);
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
               <div className="space-y-2">
                 <label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Điểm đón</label>
                 <select 
                 ...
                 >
                    ...
                    <option value={trip.route.originStation.id}>{trip.route.originStation.name} (00:00)</option>
                    {trip.route.stops
               ...
               <div className="space-y-2">
                 <label className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Điểm trả</label>
                 <select 
                    className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                 ...
                 >
                        ...
                        .filter(s => {
                            // Filter based on pickup order
                             ...
                             if (pickupStationId === trip.route.originStation.id) pickupOrder = 0;
                                 ...
                                 const pStop = trip.route.stops.find(st => st.station.id === pickupStationId);
                    ...
                    <option value={trip.route.destinationStation.id}>{trip.route.destinationStation.name} (Cuối tuyến)</option>
                 </select>
               </div>
            </CardContent>
          </Card>
