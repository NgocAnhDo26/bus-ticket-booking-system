...
import { Bus, MapPin, Wifi, Wind, Coffee } from "lucide-react";
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1 space-y-4">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col items-center px-2">
                        <span className="text-xs text-muted-foreground">{hours}h{minutes}m</span>
                    ...
                    <h3 className="text-xl font-bold text-muted-foreground">{format(arrival, "HH:mm")}</h3>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4 pt-2">
                ...
                <Badge className="text-xs">
                    {trip.bus.busLayout?.totalSeats ?? "?"} chỗ
                </Badge>
            </div>
          </div>
          ...
          <div className="flex flex-col justify-between items-end border-l pl-6 gap-4 min-w-[200px]">
            ...
            <div className="flex gap-2">
                {trip.bus.amenities.includes("WiFi") && <Wifi className="w-4 h-4 text-muted-foreground" />}
                {trip.bus.amenities.includes("Máy lạnh") && <Wind className="w-4 h-4 text-muted-foreground" />}
                {trip.bus.amenities.includes("Nước uống") && <Coffee className="w-4 h-4 text-muted-foreground" />}
            </div>
